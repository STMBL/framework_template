...
volatile struct adc12_struct_t adc12_buffer;

volatile struct adc34_struct_t adc34_buffer;
...
// TODO TIM1->CR1.ARPE
  ...
  TIM1->RCR = 11; // 30khz pwm -> 5khz rt



  GPIOC->MODER |= GPIO_MODER_MODER6_0 | GPIO_MODER_MODER7_0;
  GPIOC->AFR[1] |= /*(GPIO_AF4_TIM8 << GPIO_AFRH_AFRH6_Pos) | */(GPIO_AF4_TIM8 << GPIO_AFRH_AFRH7_Pos);

  // RCC->APB2ENR |= RCC_APB2ENR_TIM8EN; // clock en
  // TIM8->CR1 |= TIM_CR1_CMS_0 | TIM_CR1_CMS_1; // center aligned 3
  // TIM8->ARR = HAL_RCC_GetHCLKFreq() / 10000;
  // TIM8->CCMR1 |= /*TIM_CCMR1_OC1PE |*/ TIM_CCMR1_OC2PE; // preload enable
  // TIM8->CCMR1 |= TIM_CCMR1_OC2M_0 | TIM_CCMR1_OC2M_1 | TIM_CCMR1_OC2M_2; // pwm mode 3
  // TIM8->CCER |= /*TIM_CCER_CC1E |*/ TIM_CCER_CC2E; // channel enable
  // TIM8->BDTR |= TIM_BDTR_OSSI; // force low in disable state (brk)
  // TIM8->BDTR |= TIM_BDTR_OSSR; // enable state
  // TIM8->BDTR |= 0xa << TIM_BDTR_BKF_Pos; // brk filter
  // TIM8->BDTR |= TIM_BDTR_BKP; // brk polarity high
  // TIM8->BDTR |= TIM_BDTR_BKE; // brk enable
  ...
  // gpio adc
  GPIOA->MODER |= GPIO_MODER_MODER0_1 | GPIO_MODER_MODER0_0 | GPIO_MODER_MODER1_1 | GPIO_MODER_MODER1_0 | GPIO_MODER_MODER5_1 | GPIO_MODER_MODER5_0 | GPIO_MODER_MODER6_1 | GPIO_MODER_MODER6_0 | GPIO_MODER_MODER7_1 | GPIO_MODER_MODER7_0;
  GPIOC->MODER |= GPIO_MODER_MODER0_1 | GPIO_MODER_MODER0_0 | GPIO_MODER_MODER1_1 | GPIO_MODER_MODER1_0 | GPIO_MODER_MODER2_1 | GPIO_MODER_MODER2_0 | GPIO_MODER_MODER3_1 | GPIO_MODER_MODER3_0 | GPIO_MODER_MODER4_1 | GPIO_MODER_MODER4_0 | GPIO_MODER_MODER5_1 | GPIO_MODER_MODER5_0;
  GPIOF->MODER |= GPIO_MODER_MODER4_1 | GPIO_MODER_MODER4_0;
  ...
  // 3 disc. conversions
  // ADC1->CFGR |= (1 - 1) << ADC_CFGR_DISCNUM_Pos;
  // ADC2->CFGR |= (1 - 1) << ADC_CFGR_DISCNUM_Pos;
  // ADC3->CFGR |= (1 - 1) << ADC_CFGR_DISCNUM_Pos;
  ...
  // pb0 but_cur opamp in
  // pb1 31 but_cur opamp out

  // sample time
  ADC1->SMPR1 |= ADC_SMPR1_SMP1_2 | ADC_SMPR1_SMP2_2 | ADC_SMPR1_SMP5_2 | ADC_SMPR1_SMP6_2 | ADC_SMPR1_SMP8_2 | ADC_SMPR1_SMP9_2;

  ADC2->SMPR1 |= ADC_SMPR1_SMP2_2 | ADC_SMPR1_SMP3_2 | ADC_SMPR1_SMP4_2 | ADC_SMPR1_SMP5_2 | ADC_SMPR1_SMP7_2;
  ...
  // groups
  ADC1->SQR1 |= (9 << ADC_SQR1_SQ1_Pos) | (8 << ADC_SQR1_SQ2_Pos) | (6 << ADC_SQR1_SQ3_Pos) | (1 << ADC_SQR1_SQ4_Pos);
  ADC1->SQR2 |= (2 << ADC_SQR2_SQ5_Pos) | (5 << ADC_SQR2_SQ6_Pos);
  ...
  ADC2->SQR1 |= (7 << ADC_SQR1_SQ1_Pos) | (5 << ADC_SQR1_SQ2_Pos) | (11 << ADC_SQR1_SQ3_Pos) | (4 << ADC_SQR1_SQ4_Pos);
  ADC2->SQR2 |= (2 << ADC_SQR2_SQ5_Pos) | (3 << ADC_SQR2_SQ6_Pos);
  ...
  ADC3->SQR1 |= (1 << ADC_SQR1_SQ1_Pos) | (1 << ADC_SQR1_SQ2_Pos) | (1 << ADC_SQR1_SQ3_Pos) | (1 << ADC_SQR1_SQ4_Pos);
  ADC3->SQR2 |= (1 << ADC_SQR2_SQ5_Pos) | (1 << ADC_SQR2_SQ6_Pos);
  ...
  // p -> m, circ, 12x
  DMA1_Channel1->CNDTR = sizeof(adc12_buffer) / 4;
  DMA1_Channel1->CMAR = &adc12_buffer;
  ...
  DMA2_Channel5->CNDTR = sizeof(adc34_buffer) / 2;
  DMA2_Channel5->CMAR = &adc34_buffer;
  DMA2_Channel5->CPAR = &(ADC3->DR);
  DMA2_Channel5->CCR |= DMA_CCR_PL_1 | DMA_CCR_PL_0 | DMA_CCR_MSIZE_0 | DMA_CCR_PSIZE_0 | DMA_CCR_MINC | DMA_CCR_CIRC | DMA_CCR_EN;
